...
def update_patches(branch, local_patches_branch,
    ...
    filter_out = spec.get_filter_regex()
    if not filter_out:
        log.info('No valid patch filtering regex found in the spec file.')
    if filter_out and not patches_base:
        raise exception.OnlyFilterOutUsed()
    ...
    if filter_out:
        ranges = _partition_patches(patches, filter_out)
    ...
    if patches and filtered_patches:
        ...
        for patch_range in ranges:
            ...
            rng = git.rev_diff(start_commit + '~', end_commit)
