...
class Spec(object):
    ...
    def find_subpkg_by_name(self, subpkg_name=''):
        """
        ...
        subpackage within the .spec file.
        """
        beginning_of_subpkg = ''
        txt_list = self._txt.split('\n')
        all_subpkgs = re.findall(r'^%package.*$',
                                 self.txt,
                                 re.M)
        if not all_subpkgs:
            return None
        if subpkg_name:
            for subpkg in all_subpkgs:
                if subpkg.endswith(subpkg_name):
    ...
    def find_last_dependency(self, dep_type, starting_index='',
                             ending_index=''):
        """
        ...
        if starting_index != '' and ending_index != '':
            txt_range = txt_list[starting_index:ending_index]
        else:
            txt_range = txt_list

        nested_if_statement = 0
        for line in txt_range:
