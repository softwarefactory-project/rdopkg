    ...
    try:
        for cr in missing:
            if cr.desired_vers:
                ...
                requires = cr.name
            is_autosync = spec.add_python_requires(requires)
            cr.autosync = True if is_autosync else False
        for cr in wrong_version:
            is_autosync = spec.edit_python_requires_version_by_name(
                cr.name,
                cr.desired_vers)
            cr.autosync = True if is_autosync else False
        for cr in removed:
            is_autosync = spec.remove_python_requires_by_name(cr.name)
            cr.autosync = True if is_autosync else False
    except Exception:
        pass
    spec.save()
