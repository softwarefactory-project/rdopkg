...
def new_build(profile='cbs', scratch=True):
    ...
    if not build_target:
        raise exception.CantGuess(what="build tag", why="unknown branch")
    ...
    if not srpm:
        raise exception.FileNotFound(msg="No SRPM found")
...
def retrieve_sources():
    ...
    cmdline = [spectool, "-g", specfile]
    output = cmd.run(*cmdline, direct=True)
    log.warn(output)
