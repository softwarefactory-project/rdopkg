...
def test_set_magic_comment_remove_comment():
    spec = specfile.Spec(txt='# patches_base=1.2.3\n# patches_ignore=foo\n'
                             '# magic_comment=bar\n')
    spec.set_magic_comment('patches_ignore', None)
    assert 'bar' == spec.get_magic_comment('magic_comment')
...
def test_create_nmc_foo_existing_other_magic_comment_with_ordering():
    mock_file = '\n'.join(['', source_leader, '#',
                           magic_comment_patches_base, '#', ''])
...
def test_create_nmc_foo_existing_other_mc_and_patches_with_ordering():
...
def test_create_nmc_foo_existing_magic_comment_no_extra_lines():
    mock_file = '\n'.join(['', source_leader, '#', magic_comment_patches_base,
                           '#', ''])
    spec = specfile.Spec(txt=mock_file)
    spec._create_new_magic_comment('foo', 'bar')
    ...
    assert len(lines[lines.index(magic_comment_foo) + 1:
                     lines.index(magic_comment_patches_base)]) == 0


def test_create_nmc_foo_existing_magic_comment_and_patch_no_extra_lines():
    mock_file = '\n'.join(['', source_leader, '#',
                           magic_comment_patches_base, '#', patches_leader,
                           ''])
    spec = specfile.Spec(txt=mock_file)
    spec._create_new_magic_comment('foo', 'bar')
    ...
    assert len(lines[lines.index(magic_comment_foo) + 1:
               lines.index(magic_comment_patches_base)]) == 0
    assert len(lines[lines.index(magic_comment_patches_base) + 1:
               lines.index(magic_comment_foo)]) == 0
