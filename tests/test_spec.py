...
def test_find_last_dependency_1(tmpdir):
    ...
    spec = specfile.Spec(txt=txt)
    assert spec.find_last_dependency('Requires') == 4
...
def test_find_last_dependency_3(tmpdir):
    ...
    spec = specfile.Spec(txt=txt)
    assert spec.find_last_dependency('Requires', 0, 2) == 1
...
def test_find_last_dependency_4(tmpdir):
    txt = '\n'.join(['Requires:      python-foo1',
                     ...
                     '%endif'])
    spec = specfile.Spec(txt=txt)
    assert spec.find_last_dependency('Requires') == 1


def test_find_subpkg_by_name_1(tmpdir):
    txt = '\n'.join(['%package -n        python-foo',
                     ...
                     '%{common_desc}'])
    spec = specfile.Spec(txt=txt)
    assert spec.find_subpkg_by_name() == (0, 4)


def test_find_subpkg_by_name_2(tmpdir):
    txt = '\n'.join(['',
                     '%package     doc',
                     'Summary:     this is documentation section',
                     'Requires:    python3-sphinx',
                     '%description doc',
                     '%{common_desc}'])
    spec = specfile.Spec(txt=txt)
    assert spec.find_subpkg_by_name() == (1, 4)


def test_find_subpkg_by_name_3(tmpdir):
    txt = '\n'.join(['Requires:     python3-foo1',
                     ...
                     'BuildRequires:   python-baz1'])
    spec = specfile.Spec(txt=txt)
    assert spec.find_subpkg_by_name() is None


def test_find_subpkg_by_name_4(tmpdir):
    txt = '\n'.join(['%package -n        python-foo',
                     'Summary:           foo summary',
                     'BuildRequires:     python-devel',
                     'Requires:          python-futurist',
                     ...
                     '%{common_desc}'])
    spec = specfile.Spec(txt=txt)
    assert spec.find_subpkg_by_name('python-foo-tests') == (7, 11)


def test_find_subpkg_by_name_5(tmpdir):
    txt = '\n'.join(['%package -n        python-foo',
                     'Summary:           foo summary',
                     'BuildRequires:     python-devel',
                     'Requires:          python-futurist',
                     ...
                     '%{common_desc}'])
    spec = specfile.Spec(txt=txt)
    assert spec.find_subpkg_by_name('doc') is None
