...
More use cases can be found in link:fix.feature.7.html[rdopkg_fix(7)].
...
More use cases can be found in link:patch.feature.7.html[rdopkg_patch(7)].
...
*Flow:*
  ...
  * Update patches from the local patches branch (`update-patches` action)
  * Display the diff.
...
More use cases can be found in link:newversion.feature.7.html[rdopkg_new-version(7)].
...
Update `.spec` file with patches from the `patches` branch.

This is a core low level action used by other actions such as `patch` and
`new-version` to update dist-git patches from the `patches` branch.
See <<patches-branch,AUTOMAGIC -> patches branch>> for further details.

`update-patches` is a rework of the now obsolete `update-patches.sh` script with
 ...
 * Export patches from the `patches` branch using `git format-patch`
...
link:actions.feature.7.html[rdopkg_actions(7)],
link:fix.feature.7.html[rdopkg_fix(7)],
link:newversion.feature.7.html[rdopkg_newversion(7)],
link:patch.feature.7.html[rdopkg_patch(7)],
link:pkgenv.feature.7.html[rdopkg_pkgenv(7)],
